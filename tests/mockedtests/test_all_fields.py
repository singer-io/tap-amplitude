...
def _entry():
    ...
    return CatalogEntry(
        ...
        metadata=[
            ...
            {"breadcrumb": ["properties", "SERVER_UPLOAD_TIME"], "metadata": {"selected": True}},
            {"breadcrumb": ["properties", "EXTRA"], "metadata": {"selected": True}},
        ],
...
class TestAllFields(unittest.TestCase):
    ...
    def test_all_fields(self, _mock_transformer, mock_record_counter, mock_write_record, _mock_write_bookmark, _mock_write_state):
        counter = _DummyCounter()
        mock_record_counter.return_value = _DummyCounterContext(counter)

        rows = [("id-1", datetime.datetime(2026, 1, 1, 0, 0, 0), "value")]
        connection = _Connection(rows)
        entry = _entry()
        state = {"bookmarks": {"PUBLIC-events": {"SERVER_UPLOAD_TIME": None}}}

        incremental.sync_table(connection, entry, state, ["UUID", "SERVER_UPLOAD_TIME", "EXTRA"])

        written_record = mock_write_record.call_args[0][1]
        self.assertSetEqual(set(written_record.keys()), {"UUID", "SERVER_UPLOAD_TIME", "EXTRA"})
